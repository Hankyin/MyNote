...
图像锐化：突出或增强图像中的细节，增强图像边缘，增强图像边界。以便于提取图像边界，进行图形分割。

图像的边缘的特点：通常，边缘上的灰度变化平缓，而边缘两侧灰度变化较快。局部不连续，亮度变化最显著。

常见边缘类型：阶梯状，脉冲状，屋顶状。

锐化处理可以使用空间微分来完成。

用差分定义一元函数f(x)的一阶微分：dx = f(x + 1) - f(x)

用差分定义一元函数fx的二阶微分dx2 = f(x+1)+f(x-1)-2f(x)

图像的双向微分：水平和垂直两个方向同时求微分，然后对得到的两个微分值平方求和再开方即可。这也就是图像的梯度了。

锐化操作可以使用模板在图像上卷积进行计算。
...
常用的算子有：Roberts,sobel,prewitt,laplacian,LOG

图像像素值变化大为高频，变化小的为低频，所以边缘为高频信息，